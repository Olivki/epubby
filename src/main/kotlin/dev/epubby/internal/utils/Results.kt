...
internal fun <T> malformedFailure(message: String, cause: Throwable? = null): Try<T> =
    Either.Left(MalformedBookException(message, cause))

internal fun <T> missingAttribute(
    element: Element,
    attributeName: String,
    cause: Throwable? = null
): Try<T> = malformedFailure(
    "Element ${element.encodeToString(compactXmlOutputter)} is missing required attribute '$attributeName'.",
    cause,
)

internal fun <T> Try<T>.unwrap(): T = getOrHandle { throw it }

internal fun <T> missingChild(
    element: Element,
    childName: String,
    cause: Throwable? = null
): Try<T> = malformedFailure(
...
internal fun <T> Sequence<Try<T>>.flatMapFailure(): Sequence<Try<T>> {
    val successes = this.filterIsInstance<Either.Right<T>>()
    ...
    return this.filterIsInstance<Either.Left<Throwable>>()
        .map { it.value }
...
// TODO: replace with ValidatedNel
private fun PersistentList<MalformedBookException>.flatMapToFailure(): Either.Left<Throwable> {
    ...
    return when (exceptions.size) {
        1 -> Either.Left(exceptions.first())
        else -> Either.Left(MalformedBookExceptionList(exceptions))
    }
}
...
// iterable
internal inline fun <T, R> Iterable<T>.tryMap(mapper: (T) -> R): List<Try<R>> = map { Either.catch { mapper(it) } }

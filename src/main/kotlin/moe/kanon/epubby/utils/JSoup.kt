 ...
 */
fun Element.isTextEmpty(): Boolean = this.text().trim().isEmpty()

/**
 * Returns whether or not the trimmed version of `this` [Element]'s [ownText()][Element.ownText] is empty.
 */
fun Element.isOwnTextEmpty(): Boolean = this.ownText().trim().isEmpty()
 ...
 */
infix fun Element.matches(query: String): Boolean = this.`is`(query)
 ...
 */
fun Element.hasClass(): Boolean = this.hasClass(this.className())
 ...
 */
fun Element.isEmpty(): Boolean = this.children().size <= 0 && this.isOwnTextEmpty()
 ...
 */
fun Element.isParentEmpty(): Boolean =
...
): Boolean = this.allElements.filter { !(it matches query) }
    .any { element ->
...
// Elements
/**
 * Returns whether or not `this` [Elements] has any children, if it doesn't, this returns `false`, otherwise it returns
 * `true`.
 */
 ...
 */
inline fun Document.defaultOutputSettings() {
